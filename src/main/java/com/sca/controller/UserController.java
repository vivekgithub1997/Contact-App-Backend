...
public class UserController {
	...
	public ResponseEntity<String> updateContact(@PathVariable int userId, @RequestBody Contact contact) {
		logger.info("Request to update contact with id: {} for userId: {}", contact.getCId(), userId);

		String result = contactService.updateContact(userId, contact);

		if ("Contact updated successfully".equals(result)) {
			logger.info("Contact updated successfully. contactId: {}, userId: {}", contact.getCId(), userId);
			return ResponseEntity.ok(result);
		} else if (result.startsWith("User not found") || result.startsWith("Contact not found")) {
			logger.warn("Update failed for contactId: {}, userId: {}. Reason: {}", contact.getCId(), userId, result);
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(result);
		} else {
			logger.error("Unexpected error while updating contact. contactId: {}, userId: {}. Message: {}",
					contact.getCId(), userId, result);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Unexpected error.");
		}
	}
}